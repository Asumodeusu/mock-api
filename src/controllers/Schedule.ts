...
export const ScheduleController = new Elysia()
    ...
    return { data: db.data.schedule };}) // все данные выдает - добавил для понимания и на будующее
  ...
  if (!schedule.employeeIds?.length || !schedule.startDate || !schedule.endDate) {
    return false;  // Проверяем что есть данные для фильтрации (от обратного идем)
  } return (
